...
export const action = async ({ request, params: { clubId, newsId } }: ActionArgs) => {
  ...
  const { title, body, imageUrls } = validation.data;

  try {
    const clubNewsItem = await updateClubNews(newsId, title, body, formData.get('isPublic') === 'on', imageUrls.split(','), clubId);
    ...
    <React.Fragment>
      <div className={'flex justify-center py-4'}>
        <Form method={'post'} className={'w-full max-w-xl'}>
          ...
          <input name="imageUrls" id="imageUrls" hidden defaultValue={newsItem.imageUrls.map(imageUrl => imageUrl.url).join(',')} />
